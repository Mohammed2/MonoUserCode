...
void TrackCombinerReco::FitXY(vector<int> &Group){
  ...
  float XMax = 0;
  float Phi0 = _Points[0].phi(); // rotate so the initial path is along the x axis
  ...
  _XYPar[1] = Phi0 + atan(Result->Parameter(1)/Result->Parameter(0)); // phi0
  _XYErr[1] = Result->ParError(1); // not correct right now
  _XYPar[2] = Result->Parameter(2); // curvature (1/radius)
